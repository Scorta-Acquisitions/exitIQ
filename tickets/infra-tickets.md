...
- [ ] `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` are exposed to the client bundle via the `client` block in `env.mjs`
- [ ] `DATABASE_URL`, `SUPABASE_URL`, and `SUPABASE_SERVICE_SECRET_KEY` are server-only
...
**Supabase values to retrieve from project dashboard (Settings → API):**
- `Project URL` → `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_URL`
- Publishable key (legacy label: `anon` / public) → `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`
- Secret key (legacy label: `service_role`) → `SUPABASE_SERVICE_SECRET_KEY`
