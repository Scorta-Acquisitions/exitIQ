...
test.describe("who sees what: the demo plays itself", () => {
  test("opens the record one level at a time, rests on its end state, then plays again", async ({ page }) => {
    ...
    await expect(level(page)).toHaveText("Level 4 · Final diligence · Visible 6 of 7")
    await expect(page.getByTestId("priv-log").locator("li")).toHaveCount(3)
    await settled(page)
    const ended = await frameHeight(page)
    await expectReplay(page, "priv-demo", "rules")
    await expect(level(page)).toHaveText("Level 0 · Nothing public · Visible 0 of 7")
