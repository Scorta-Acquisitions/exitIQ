...
/**
 * The "instrument field": a WebGL fractal-noise backdrop behind the hero console and the exitIQ run.
 * `target` (0..1) brightens the field as the visitor's readiness signal grows; `pulse()` adds a short
 * flare when an answer lands. Falls back silently when WebGL is unavailable. Renders one static frame
...
export function useInstrumentField(canvasRef: RefObject<HTMLCanvasElement | null>, target: number) {
