...
beforeAll(async () => {
  const sessionRoute = await import("@/legacy/app/api/assessment/session/route")
  POST = sessionRoute.POST as PostHandler
  const sessionGetRoute = await import("@/legacy/app/api/assessment/session/[session_id]/route")
